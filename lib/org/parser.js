...
InlineParser.prototype = {
  ...
  _getMatchesForTextWithPattern: function(text, pattern, onMatch, nextParser) {
    ...
    var result = [],
      ...
      while(match = myPattern.exec(text)) {
        ...
        if(maybeNode) {
          ...
          result.push(nextParser.call(this, match[0]))
        }
  ...
  parseTimestamp: function(text) {
    return this._getMatchesForTextWithPattern(
      ...
      function(match) {
        ...
        var timestamp = Node.createTimestamp([
          Node.createText(null, {value: whole})
        ]);
